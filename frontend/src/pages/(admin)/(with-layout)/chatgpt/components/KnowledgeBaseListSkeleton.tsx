...
const KnowledgeBaseListSkeletonItems: React.FC<{ itemCount?: number }> = ({
  itemCount = 5,
}) => {
  return (
    <>
      {[...Array(itemCount)].map((_, i) => (
        <div key={i} className="flex items-center space-x-3 py-2 px-2">
          <SkeletonElement className="h-10 w-10 rounded-md" />
...
export default KnowledgeBaseListSkeletonItems;
