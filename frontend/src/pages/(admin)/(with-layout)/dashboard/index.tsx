...
const transformInsightTrends = (apiData: InsightTrendsData): { chartData: TransformedChartDataItem[], chartConfig: ChartConfig } => {
  if (!apiData || !apiData.labels || !apiData.datasets) {
    return { chartData: [], chartConfig: {} };
  }

  const transformedData: TransformedChartDataItem[] = apiData.labels.map((label, index) => {
...
export function Component() {
  ...
  const [insightTrends, setInsightTrends] = useState<{ chartData: TransformedChartDataItem[], chartConfig: ChartConfig }>({ chartData: [], chartConfig: {} });
  const [issuesData, setIssuesData] = useState<IssueItem[]>([]);
  const [issuesPagination, setIssuesPagination] = useState<PaginationInfo | undefined>(undefined);
  // Add loading and error states as needed
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const developerId = "test_id2"; // Using developer_id from test.ts

  // Mock data is kept for reference or quick fallback if needed, but won't be used by default.
  // const MOCK_KPI_DATA: KpiSummary = { ... };
  // const MOCK_INSIGHT_TRENDS_DATA: InsightTrendsApiResponseData = { ... };
  // const MOCK_ISSUES_DATA: IssueItem[] = [ ... ];
  // const MOCK_ISSUES_PAGINATION: PaginationInfo = { ... };

  useEffect(() => {
    const USE_MOCK_DATA = false; // Set to false to use actual API calls
    ...
    const fetchData = async () => {
      ...
      try {
        if (USE_MOCK_DATA) {
          // Kept for fallback, e.g., by manually setting USE_MOCK_DATA to true for debugging
          // setKpiData(MOCK_KPI_DATA);
          // setInsightTrends(transformInsightTrends(MOCK_INSIGHT_TRENDS_DATA)); 
          // setIssuesData(MOCK_ISSUES_DATA);
          // setIssuesPagination(MOCK_ISSUES_PAGINATION);
          console.warn("Using MOCK DATA for dashboard! Switch USE_MOCK_DATA to false for live data.");
        } else {
          // Fetch KPI Summary
          const kpiRes = await fetch(`${import.meta.env.VITE_SERVER_HOST}/developers/dashboard/kpi-summary`, { // Updated URL
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ developer_id: developerId }),
          });
          const kpiResult: KpiSummaryResponse = await kpiRes.json();
          if (!kpiRes.ok || !kpiResult.success || !kpiResult.data) { // Added kpiRes.ok check
            throw new Error(kpiResult.message || `Failed to fetch KPI summary (HTTP ${kpiRes.status})`);
          }
          setKpiData(kpiResult.data);

          // Fetch Insight Trends
          const trendsPayload = { developer_id: developerId, period: "90d", granularity: "daily" }; 
          const trendsRes = await fetch(`${import.meta.env.VITE_SERVER_HOST}/developers/dashboard/insight-trends`, { // Updated URL
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(trendsPayload),
          });
          const trendsResult: InsightTrendsResponse = await trendsRes.json();
          if (!trendsRes.ok || !trendsResult.success || !trendsResult.data) { // Added trendsRes.ok check
            throw new Error(trendsResult.message || `Failed to fetch insight trends (HTTP ${trendsRes.status})`);
          }
          const { labels, datasets } = trendsResult.data;
          setInsightTrends(transformInsightTrends({ labels, datasets }));
          
          // Fetch Issues Data
          const issuesPayload = { developer_id: developerId, page: 1, limit: 10, sortBy: "last_seen_at", sortOrder: "desc" }; 
          const issuesRes = await fetch(`${import.meta.env.VITE_SERVER_HOST}/developers/profile/issues`, { // Updated URL
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(issuesPayload),
          });
          const issuesResult: PaginatedIssuesResponse = await issuesRes.json();
          if (!issuesRes.ok || !issuesResult.success || !issuesResult.data || !issuesResult.pagination) { // Added issuesRes.ok check
            throw new Error(issuesResult.message || `Failed to fetch issues (HTTP ${issuesRes.status})`);
          }
  ...
  if (error) {
    return <div className="p-6 text-red-600"><p>Error loading data: {error} (Make sure Koa server is running on port 4000 and check console)</p></div>;
  }
