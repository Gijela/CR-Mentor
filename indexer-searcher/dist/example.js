...
const index_state_1 = require("./core/index-state");
async function main() {
    // 假设 CodeIndexer 和 CodeSearcher 的构造函数现在接受 Config 类型
    // (我们可能需要稍后调整 Config 和构造函数)
    const indexer = new indexer_1.CodeIndexer({
        index: { root: path_1.default.join(__dirname, '.index'), maxCPUs: 4, timeout: 600000 },
        search: { defaultLimit: 10, timeout: 30000 },
        symf: { path: path_1.default.join(__dirname, '../node_modules/.bin/symf') },
    });
    const searcher = new searcher_1.CodeSearcher(indexer);
    const stateManager = new index_state_1.IndexStateManager(path_1.default.join(__dirname, '.index'));
    try {
        const searchRoot = path_1.default.join(__dirname, '../'); // 搜索 project-B 自身的代码
        console.log(`搜索范围: ${searchRoot}`);
        const scopeDir = (0, file_1.createFileURI)(searchRoot); // 这是 FileURI
        // 检查索引状态
        ...
        const liveFile = path_1.default.join(__dirname, 'example.ts');
        console.log(`实时搜索:
    - 查询: "${liveQuery}"
    - 文件: ${liveFile}`);
        const liveStartTime = Date.now();
        const liveResults = await searcher.getLiveResults(liveQuery, 'example', [liveFile]);
